...
import info
import pandas as pd # adds table
# About Me
def about_me_section(): # define funcation, no parameters 
    st.header("❀ About Me ❀") # print header 
    st.image(info.profile_picture, width = 200) # upload photo + dimentions 
    st.write(info.about_me) # print info 
    st.write('---') # print blank line 
about_me_section() # call function 
...
def education_section(education_data, course_data):
    st.header("✎ Education")
    st.subheader(f"**{education_data['Institution']}**") # ** make bold
    st.write(f"**Degree:** {education_data['Degree']}")
    st.write(f"**Graduation Date:** {education_data['Graduation Date']}")
    st.write(f"**GPA:** {education_data['GPA']}")
    st.write("**Relevent Coursework:**")
    coursework = pd.DataFrame(course_data) # course data stored in info file
...
def experience_section(experience_data):
    ...
    for job_title, (job_description, image)in experience_data.items():
        expander = st.expander(f"{job_title}") # tabs that expand and show more info
